<?php namespace App\Http\Controllers;
	...
	class AdminCmsUsers47Controller extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->where='id_cms_privileges=3';
			...
			$this->form[] = ['label'=>'Company Name','name'=>'company_name','type'=>'text','width'=>'col-sm-10','help'=>'File types support : JPG, JPEG, PNG, GIF, BMP'];
			...
			$this->form[] = ['label'=>'City','name'=>'city','type'=>'text','width'=>'col-sm-10','datatable'=>'state,StateName'];
			$this->form[] = ['label'=>'Pincode','name'=>'pincode','type'=>'text','width'=>'col-sm-10'];
			...
			$this->form[] = ['label'=>'Privileges','name'=>'id_cms_privileges','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'cms_privileges,name',"datatable_where"=>"id=4"];
			$this->form[] = ['label'=>'Status','name'=>'status','type'=>'select','validation'=>'required','width'=>'col-sm-10','dataenum'=>'Active;In-Active'];
			...
			//$this->form[] = ["label"=>"Gst No","name"=>"gst_no","type"=>"textarea","required"=>TRUE,"validation"=>"required|string|min:5|max:5000"];
			//$this->form[] = ["label"=>"Qrcode","name"=>"qrcode","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Token","name"=>"token","type"=>"textarea","required"=>TRUE,"validation"=>"required|string|min:5|max:5000"];
			//$this->form[] = ["label"=>"Referalcode","name"=>"referalcode","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Refferby","name"=>"refferby","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Wallet","name"=>"wallet","type"=>"number","required"=>TRUE,"validation"=>"required|integer|min:0"];
			//$this->form[] = ["label"=>"Easymoney","name"=>"easymoney","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Dob","name"=>"dob","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Trade","name"=>"trade","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Industry","name"=>"industry","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Commission","name"=>"commission","type"=>"number","required"=>TRUE,"validation"=>"required|integer|min:0"];
			# OLD END FORM
	        ...
	        */
		    ...
		    $this->sub_module[] = ['label' => 'Location', 'path' => 'locations	', 'parent_columns' => 'name', 'foreign_key' => 'userId', 'button_color' => 'success', 'button_icon' => 'fa fa-location'];
		   ...
		   $this->table_row_color[] = ["condition"=>"[status] == 'In-Active'","color"=>"warning"];
	        ...
	        |
	        */
	           $this->index_statistic[] = ['label'=>'Total Active Associate','count'=>DB::table($this->table)->where('id_cms_privileges',4)->where('status','Active')->count(),'icon'=>'fa fa-user','color'=>'green','width'=>'col-sm-3'];

			$this->index_statistic[] = ['label'=>'Total In-Active Associate','count'=>DB::table($this->table)->where('id_cms_privileges',4)->where('status','In-Active')->count(),'icon'=>'fa fa-user','color'=>'red','width'=>'col-sm-3'];
